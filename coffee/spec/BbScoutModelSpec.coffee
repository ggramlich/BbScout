...
describe 'Game', ->
	...
	it 'holds both teams', ->
		expect(game.teamA).toBe teamA
		expect(game.getTeam('Team A')).toBe teamA
		expect(game.teamB).toBe teamB
		expect(game.getTeam('Team B')).toBe teamB

	it 'computes the score', ->
		spyOn(teamA, 'points').andReturn 10
		spyOn(teamB, 'points').andReturn 25
		expect(game.score()).toBe '10:25'
		expect(teamA.points).toHaveBeenCalled()
		expect(teamB.points).toHaveBeenCalled()
